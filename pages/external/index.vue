<template>
  ...
  <div v-else>
    ...
    <div class="container">
      <div class="row">
        <div v-if="external && external.length" class="col-12 col-md-9 pd">
          ...
          >
            <nuxt-link :to="localePath(`/external/${item.id}`)">
              <h2>{{ item.title }}</h2>
...
export default {
  ...
  computed: {
    links() {
      return [
        {
          title: this.$t('correspondence'),
          link: `${this.$i18n.locale}`,
        },
      ]
    },
    ...mapState({
      external: (state) => state.courses.external,
      perPage: (state) => state.courses.external.perPage,
      total: (state) => state.courses.external.total,
      currentPage: (state) => state.courses.external.currentPage,
    }),
  },
  async fetch() {
    await this.$store.dispatch('courses/fetchExternal').catch(() => {})
  },
}
</script>
