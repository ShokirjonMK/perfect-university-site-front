        ...
        <div>
          <div class="pd">
            ...
            <div
              v-if="
                graduatorList &&
                graduatorList.results &&
                graduatorList.results.length
              "
            >
              <div
                v-for="(item, index) in graduatorList.results"
                :key="index"
                class="graduatorsCards"
              >
                <nuxt-link
                  :to="`/${$i18n.locale}/students/alumni/${item.slug}`"
                >
                    ...
                    <div class="w-100" style="padding: 0">
                      <div class="content">
                        ...
                        <div class="alumni__link">
                          <div class="ps-2">
                            <p v-if="item.faculty" class="item__faculty">
                              <svg
                                width="20"
                                height="20"
                                viewBox="0 0 20 20"
                            ...
                            <p v-if="item.year" class="item__year">
                              <svg
                                width="20"
                                height="20"
                                viewBox="0 0 20 20"
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      graduatorList: (state) => state.graduators.graduatorList,
      total: (state) => state.graduators.graduatorList.total,
      perPage: (state) => state.graduators.graduatorList.per_page,
      currentPage: (state) => state.graduators.graduatorList.current_page,
    }),
  },
  ...
  methods: {
    async handlePaginationChange(newPage) {
      await this.$store
        .dispatch('graduators/fetchGraduatorList', { page: newPage })
        .catch(() => {})
      this.scrollToTop()
    },
  ...
  async fetch() {
    await this.$store
      .dispatch('graduators/fetchGraduatorList', { page: this.page })
      .catch(() => {})
  },
}
</script>
