<template>
  <div>
    ...
    <div v-else>
      ...
      <div class="container">
        <div v-if="conferenceSlug" class="row">
          <div class="col-12 col-md-9 structure-styles pd">
            <h1>
              {{ conferenceSlug.title ? conferenceSlug.title : '' }}
            </h1>

            <img :src="conferenceSlug.image_url" alt="Conference slug" />

            <div class="single-content" v-html="conferenceSlug.content"></div>
            <h2 v-if="conferenceSlug.files && conferenceSlug.files.length">
              {{ $t('related_docs') }}
            </h2>
            <template
              v-if="conferenceSlug.files && conferenceSlug.files.length"
            >
              <div
                v-for="(item, index) in conferenceSlug.files"
                :key="index"
      ...
      return [
        {
          ...
          link: `/${this.$i18n.locale}/international/international-conferences`,
        },
        {
          title: this.conferenceSlug?.title,
          link: `${this.$i18n.locale}`,
        },
      ]
    },
    ...mapState({
      conferenceSlug: (state) =>
        state.international.internationalConferencesSlug,
    }),
  },
  async fetch() {
    await this.$store
      .dispatch(
        'international/fetchInternationalConferencesSlug',
