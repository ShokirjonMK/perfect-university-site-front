<template>
  ...
  <div v-else>
    ...
    <!-- <pre>{{scientificMagazines}}</pre> -->
    <div class="container">
      <div class="row">
        <div
          v-if="scientificMagazines && scientificMagazines.length"
          class="col-12 pd"
        >
          <h1>{{ $t('scientific_journals') }}</h1>

          <div
            v-for="(item, index) in scientificMagazines"
            :key="index"
            class="cardsGrants mb-4"
          >
            <NuxtLink
              :to="localePath(`/science/scientific-magazines/${item.id}`)"
            >
              <h2>{{ item.title }}</h2>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      scientificMagazines: (state) => state.report.scientificMagazines,
  ...
  async fetch() {
    await this.$store
      .dispatch('report/fetchScientificMagazines')
      .catch(() => {})
  },
}
</script>
