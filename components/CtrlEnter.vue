<template>
  <div>
    <b-modal
      id="ctrl-enter"
      ref="ctrl-enter"
      class="ctrl-enter"
      size="lg"
      centered
    >
      <div class="modal-head">
        <h6>{{ $t('report_bug') }}</h6>
        <button class="btn btn-close" @click="$bvModal.hide('ctrl-enter')">
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M10 0C15.5228 0 20 4.47715 20 10C20 15.5228 15.5228 20 10 20C4.47715 20 0 15.5228 0 10C0 4.47715 4.47715 0 10 0ZM13.5303 6.46967L13.4462 6.39705C13.1852 6.2034 12.827 6.20101 12.5636 6.38988L12.4697 6.46967L10 8.939L7.53033 6.46967L7.44621 6.39705C7.18522 6.2034 6.82701 6.20101 6.56362 6.38988L6.46967 6.46967L6.39705 6.55379C6.2034 6.81478 6.20101 7.17299 6.38988 7.43638L6.46967 7.53033L8.939 10L6.46967 12.4697L6.39705 12.5538C6.2034 12.8148 6.20101 13.173 6.38988 13.4364L6.46967 13.5303L6.55379 13.6029C6.81478 13.7966 7.17299 13.799 7.43638 13.6101L7.53033 13.5303L10 11.061L12.4697 13.5303L12.5538 13.6029C12.8148 13.7966 13.173 13.799 13.4364 13.6101L13.5303 13.5303L13.6029 13.4462C13.7966 13.1852 13.799 12.827 13.6101 12.5636L13.5303 12.4697L11.061 10L13.5303 7.53033L13.6029 7.44621C13.7966 7.18522 13.799 6.82701 13.6101 6.56362L13.5303 6.46967L13.4462 6.39705L13.5303 6.46967Z"
              fill="#063F62"
            />
          </svg>
        </button>
      </div>

      <div class="modal-my-body">
        <b-form @submit="onSubmit">
          <input
            id="input-url"
            v-model="form.page"
            type="hidden"
            placeholder=""
          />

          <b-form-group
            id="input-group-1"
            :label="$t('error')"
            label-for="input-1"
          >
            <b-form-input
              id="input-1"
              v-model="form.title"
              type="text"
              required
              :placeholder="$t('error')"
            ></b-form-input>
          </b-form-group>
          <b-form-group :label="$t('your_comment')" label-for="input-1">
            <b-form-textarea
              id="textarea"
              v-model="form.comment"
              :placeholder="$t('your_comment')"
              class="textarea-comment"
            ></b-form-textarea>
          </b-form-group>

          <button type="submit" class="btn btn-blue">{{ $t('send') }}</button>
        </b-form>
      </div>
    </b-modal>
  </div>
</template>

<script>
export default {
  name: 'CtrlEnter',
  data() {
    return {
      form: {
        title: '',
        comment: '',
        page: '',
      },
    }
  },
  computed: {
    modal() {
      return this.$refs['ctrl-enter']
    },
  },
  mounted() {
    if (process.client) {
      document.body.addEventListener('keypress', this.handleKeypess)
      this.form.page = document.location.href
    }
  },
  methods: {
    handleKeypess(e) {
      if (e.code === 'Enter' && e.ctrlKey) {
        if (!this.modal.isShow) {
          let selectedText = ''

          if (window.getSelection) {
            selectedText = window.getSelection().toString()
          } else if (document.getSelection) {
            selectedText = document.getSelection().toString()
          } else if (document.selection) {
            selectedText = document.selection.createRange().text
          }

          this.form.title = selectedText

          this.modal.show()
        }
      }
    },
    onSubmit(evt) {
      evt.preventDefault()

      this.$axios
        .post(`typo/`, this.form)
        .then(() => {
          this.$bvModal.hide('ctrl-enter')
        })
        .finally(() => {
          this.preloader = false
        })

        .catch((error) => {})
    },
  },
}
</script>

<style lang="scss" scoped>
.textarea-comment {
  resize: vertical;
  max-height: 250px;
  min-height: 100px;
}
</style>
