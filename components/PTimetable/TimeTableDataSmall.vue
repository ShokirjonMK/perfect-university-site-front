<template>
  <div class="table-multiple">
    <template  v-for="(day,i) of data">
      <div class="table-data small" :key="i" v-if="i < show"  @click="handleDay(day)" :class="checkPeriodTime(day)">
        <div class="w-100 d-flex align-items-center justify-content-between">
          <p class="small-text">{{ day.subject }}</p>
          <svg class="small-icon" :class="{'text-white' : checkPeriodTime(day)}" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
        ...
        <div class="w-100 d-flex align-items-center justify-content-between">
          <p class="small-group">{{ day.class[0] }}</p>
          <div class="table-room small-room" :class="{'text-white' : checkPeriodTime(day)}">{{ day.classroom[0] }}</div>
        </div>
      </div>
    </template>
  </div>
