<template>
  <ul class="sidebar d-flex flex-column position-relative">
    ...
    <li
      v-for="(item, items) in undergraduateSidebarList"
      :key="items"
      ...
      <NuxtLink
        ...
        :class="{
          active:
            ...
            (items === 0 && getRoute.includes('courses/')),
        }"
...
<script>
import { undergraduateSidebarList } from '@/helpers/linkers'
export default {
  name: 'StudentSidebar',

  data() {
    return {
      undergraduateSidebarList,
    }
  },
  computed: {
    getRoute() {
